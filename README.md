# NaviX
**NaviX 是基于原同类导航项目持续改造的私有导航站，设置密码后需要输密访问，适合个人使用！**
...
3.  **配置构建**:
    ...
    *   构建命令: `npm run build`
    *   输出目录: `dist`
4.  **创建数据库**: 在 Workers & Pages -> KV 中创建一个新的命名空间，建议命名为 `NAVIX_DB`
5.  **绑定变量**:
    *   进入 Pages 项目设置 -> 绑定 (Bindings) -> 添加 KV 命名空间 -> 变量名优先填 `NAVIX_KV`，旧部署的 `CLOUDNAV_KV` 仍兼容，值选择刚才创建的 `NAVIX_DB`
...
#### 第七步：空间名称填写 `NAVIX_DB`（建议复制）
![第七步](./doc/7.png)

#### 第八步：绑定 KV 数据库
回到刚才的 pages 设置页面找到绑定-右侧下滑找到 kv 命名空间，变量名称优先填写 `NAVIX_KV`（旧版 `CLOUDNAV_KV` 仍兼容，建议复制）
![第八步](./doc/8.png)
