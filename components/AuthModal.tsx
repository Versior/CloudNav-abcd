...
interface AuthModalProps {
  ...
  onClose?: () => void; // 提供时:ESC / 点遮罩可关闭,消除无出口的死循环(#28)
}
  ...
  return (
    ...
    >
      <div
        ...
        aria-modal="true"
        aria-label="身份验证"
        className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700 p-8"
      ...
      >
        ...
        {onClose && (
          <p className="text-xs text-slate-400 dark:text-slate-500 text-center mt-4">
            无法登录?按 ESC 或点击空白处关闭;并确认部署时已设置 PASSWORD 环境变量
          </p>
        )}
      </div>
    </div>
