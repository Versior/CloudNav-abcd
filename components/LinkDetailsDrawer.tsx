...
import { X, ExternalLink, Clock, Eye, EyeOff, Activity, Tag, FileText, Bookmark, KeyRound, Plus, Trash2, Copy, Lock } from 'lucide-react';
  ...
  return (
    <div className={`fixed inset-y-0 right-0 z-40 w-96 max-w-[calc(100%-1rem)] bg-white dark:bg-slate-800 shadow-2xl border-l border-slate-200 dark:border-slate-700 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <div className="overflow-y-auto h-full pb-20 p-4 space-y-4">
        ...
        <div>
          <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1 mb-1"><FileText size={12} /> 网站笔记</label>
          <textarea value={note} onChange={e => setNote(e.target.value)} onBlur={() => onUpdate(link.id, { note })}
            className="w-full p-2 text-sm rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 resize-none" rows={4} placeholder="记录这个网站的用途、注意事项、使用技巧..."
          />
        </div>

        <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-3 space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1"><KeyRound size={12} /> 账号保险箱</label>
            <button onClick={() => setCredentialDrafts(prev => [...prev, emptyCredential()])} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-md bg-blue-600 text-white hover:bg-blue-700">
              <Plus size={12} /> 添加
            </button>
          </div>
