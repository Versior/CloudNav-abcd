...
import { X, ExternalLink, Clock, Eye, EyeOff, Activity, Tag, FileText, Bookmark, KeyRound, Plus, Trash2, Copy, Lock, Save, Eraser } from 'lucide-react';
                ...
                <div key={draft.id} className="rounded-lg border border-slate-200 dark:border-slate-700 p-3 space-y-2 bg-slate-50/50 dark:bg-slate-900/30">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate">{draft.label || draft.username || draft.account || '未命名账号'}</div>
                      <div className="text-[11px] text-slate-400">{draft.passwordCipher ? '已加密保存密码' : draft.password ? '待保存新密码' : '未保存密码'}</div>
                    </div>
