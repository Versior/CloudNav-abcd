...
const ReadingWorkspacePage: React.FC<ReadingWorkspacePageProps> = ({ initialId, onInitialIdConsumed, onOpenUrl, onNotice, initialCapture, aiConfig }) => {
  ...
  return (
    <section data-page="reading-workspace" data-layout-version="reading-workspace-v1" className="cloudnav-reading-workspace">
      <header className="cloudnav-reading-header">
        <div><div className="cloudnav-eyebrow"><BookOpen size={14} /> READING WORKSPACE</div><h1>阅读台</h1><p>收集、阅读、标注，再把真正重要的内容沉淀下来。</p></div>
      ...
      <div className="cloudnav-reading-grid">
        <aside className="cloudnav-reading-list" aria-label="阅读队列">
          <div className="cloudnav-reading-list-head"><strong>{visible.length} 条内容</strong><button type="button" onClick={() => setSelectedIds(new Set(visible.map(item => item.id)))}>全选</button></div>
        ...
        <article ref={readerRef} className="cloudnav-reading-reader" aria-label="阅读详情">
          {selected ? <>
            ...
            <div className="cloudnav-reader-footer"><button type="button" onClick={() => changeStatus('later')}><Clock3 size={15} />稍后阅读</button><button type="button" onClick={() => changeStatus('archive')}><Archive size={15} />归档</button><span><Keyboard size={14} /> `j/k` 浏览 · `l` 稍后 · `e` 归档 · `s` 收藏</span></div>
          </> : <div className="cloudnav-reading-reader-empty"><BookOpen size={36} /><strong>选择一篇内容开始阅读</strong><span>阅读进度、高亮和笔记会自动保存在本地。</span></div>}
        </article>
      </div>
