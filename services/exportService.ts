...
`;
  ...
  categories.forEach(cat => {
    ...
    catLinks.forEach(link => {
      ...
      const iconAttr = link.icon ? ` ICON="${escapeAttr(link.icon)}"` : '';
      html += `        <DT><A HREF="${escapeAttr(link.url)}" ADD_DATE="${date}"${iconAttr}>${escapeHtml(link.title)}</A>\n`;
      if (link.description) html += `        <DD>${escapeHtml(link.description)}\n`;
    });
  ...
  if (uncategorized.length > 0) {
    ...
    uncategorized.forEach(link => {
        const date = Math.floor(link.createdAt / 1000);
        html += `        <DT><A HREF="${escapeAttr(link.url)}" ADD_DATE="${date}">${escapeHtml(link.title)}</A>\n`;
        if (link.description) html += `        <DD>${escapeHtml(link.description)}\n`;
    });
    html += `    </DL><p>\n`;
  }
